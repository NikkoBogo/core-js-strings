...
function removeFirstOccurrences(str, value) {
  ...
  const lastIndex = str.indexOf(value) + value.length;
  let result;
  if (str.includes(value) === true) {
    result = str.slice(0, firstIndex) + str.slice(lastIndex);
  } else {
    result = str;
  }
  return result;
...
function removeLastOccurrences(str, value) {
  ...
  const lastIndex = str.lastIndexOf(value) + value.length;
  let result;
  if (str.includes(value) === true) {
    result = str.slice(0, firstIndex) + str.slice(lastIndex);
  } else {
...
function sumOfCodes(str) {
  ...
  if (str === '' || str === undefined || str === null) {
    ...
    while (i < str.length && str !== '' && str !== undefined) {
...
function isPalindrome(str) {
  let i = 0;
  let result = '';
  const preparedStr = str.replace(/\W|_/g, '').toLowerCase();
...
function reverseWords(str) {
  ...
  const result = array
    .map(function reverseWord(element) {
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === ' ') {
      ...
      result += '?';
    } else if (str[i].charCodeAt() > 77 && str[i].charCodeAt() < 96) {
      result += String.fromCharCode(str[i].charCodeAt() - 13);
    } else if (str[i].charCodeAt() > 109) {
      result += String.fromCharCode(str[i].charCodeAt() - 13);
    } else {
      result += String.fromCharCode(str[i].charCodeAt() + 13);
    }
